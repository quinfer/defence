...
#|label: setup
#|include: false
library(tidyverse);library(lubridate);library(kableExtra);library(corrr);library(flextable)
...
::: {#fig-data layout-ncol=1}

![Prices](plots/fig-prices.png){#fig-prices width="100%"}

![Returns](plots/fig-rtns.png){#fig-rtn width="100%"}
![Volatilites](plots/fig-vol.png){#fig-vol width="100%"}

Raw data on daily prices and returns
:::
...
dat %>% filter(volatility>0.06) |> 
  select(!stock_match_name&!price) |>
  flextable() |>
  colformat_double(digits = 4)
  # kbl(digits = 2,format = "latex",booktabs = TRUE) |>
...
labels_dat %>% rename(date=xvalues) %>% mutate(date=ymd(date)) %>% kbl() %>% kable_styling()
```
...
![Net spillover effects for stock returns at the 5th percentile](plots/fig-rtnnet5.png){#fig-rtnet5 width="100%"}

![Net spillover effects for stock returns at the 95th percentile](plots/fig-rtnnet95.png){#fig-rtnet95 width="100%"}

![Net spillover effects for stock volatilities at the median](plots/fig-volnet50.png){#fig-volnet50 width="100%"}

![Net spillover effects for stock volatilities at the 5th percentile](plots/fig-volnet5.png){#fig-volnet5 width="100%"}

![Net spillover effects for stock volatilities at the 95th percentile](plots/fig-volnet95.png){#fig-volnet95 width="100%"}
...
tbl_reg_1[-1,] |>
   kbl(col.names = names) %>%
 add_header_above(header = c(" ","Middle quantile"=2,"Upper quantile"=2,"Lower quantile"=2)) |> add_footnote("This table presents the estimated coefficients of the regression model in Equation (7) based on a covariance estimator that accounts for the presence of heteroscedasticityand autocorrelation (HAC). The sample period is 23 August 2010 –July 1, 2022.") |> kable_styling()
```
...
refinitiv %>% 
  ...
  select(Name,MktCap,RevenueFY0,RevenueFY1) %>%
   kbl(col.names = c("Company Name","Market Capitalistation","Total Revenue 2022","Total Revenue 2021"),format.args = list(big.mark = ",")) |> 
  kable_styling()
